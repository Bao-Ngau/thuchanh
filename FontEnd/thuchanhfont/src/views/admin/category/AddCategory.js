...
const AddCategory = (props) => {
    ...
    const handleSubmit = (e) => {
        ...
        axios.post(`http://localhost:8081/category/add/${props.userName}`, { name: name }, {
            headers: {
            ...
            if (response.status === 200) {
                setName("");
                toast.success("Thêm thông tin thể loại thành công");
            }
        }).catch((error) => {
            toast.error(error.response.data);
            toast.error("Thêm thông tin thể loại thất bại");
        })
    };
